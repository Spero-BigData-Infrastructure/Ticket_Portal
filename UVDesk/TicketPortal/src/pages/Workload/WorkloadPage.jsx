...
function TicketTableDrawer({ open, onClose, title, statusKey, tickets, loading, isDark, onTicketClick }) {
  ...
  return (
    <Drawer anchor="right" open={open} onClose={onClose}
      PaperProps={{ sx: { width: { xs: "100vw", md: "65vw", lg: "55vw" }, maxWidth: "900px", display: "flex", flexDirection: "column" } }}
    >
      <Paper elevation={0} sx={{
        ...
        <Box sx={{ display: "flex", alignItems: "center", flexWrap: "wrap", gap: 2, mb: 2.5 }}>
          ...
          >
            ...
            {title}
            {cfg.label && <span style={{ color: cfg.color || "#2962ff", marginLeft: 4 }}>{cfg.label}</span>}
          </Typography>
                  ...
                  <TableCell key={head} sx={{ bgcolor: hdrBg, color: isDark ? "#94a3b8" : "#64748b", fontWeight: 700, borderBottom: `1px solid ${borderClr}`, whiteSpace: "nowrap", padding: "8px 10px" }}>
                    {head === "Status" ? (
                      ...
                      >
                        ...
                        {["Open", "Pending", "Answered", "Resolved", "Closed"].map((s) => (
            ...
            <TableBody>
              {loading ? (
                ...
                >
                  ...
                  <TableCell sx={{ maxWidth: 160, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", color: "text.primary" }}>{t.type || "-"}</TableCell>
                  <TableCell sx={{ maxWidth: 140, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", color: "text.primary" }}>{t.agent_name || "-"}</TableCell>
...
// ── Breakdown Modal ────────────────────────────────────────────
function BreakdownModal({ open, onClose, projName, month, total, breakdown, isDark, onStatusClick }) {
  if (!open) return null;
  ...
  return (
    <Dialog open={open} onClose={onClose}
      PaperProps={{
        sx: {
          background: cardBg, borderRadius: "18px", border: `1px solid ${borderClr}`,
      ...
      <DialogContent sx={{ pt: 0 }}>
        ...
        <Box sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
          {Object.entries(STATUS_CONFIG).map(([key, cfg]) => {
            ...
            return (
              <Box key={key}
                onClick={() => count > 0 && onStatusClick(key, count)}
                sx={{
...
export default function WorkloadPage() {
  ...
  // API summary — direct from backend (no recompute needed)
  ...
  // lookup[month][projectName] = { total, breakdown, tickets_by_status }
  ...
  // Ticket table drawer — fed from heatmap data directly
  ...
  // ── Fetch Heatmap ──────────────────────────────────────────
  // API response shape:
  // { status, summary:{total_tickets, active_tickets, resolved_tickets, closed_tickets, total_projects},
  //   project_dropdown, months:[{ month, projects:[{ project_name, overall:{open,pending,...,total}, tickets_by_status:{open:[],..} }] }] }
  const fetchData = useCallback(async (fd, td, sp) => {
    ...
    try {
      ...
      console.log("[WorkloadPage] API res:", JSON.stringify({ status: res?.status, summaryKeys: Object.keys(res?.summary || {}), monthsCount: res?.months?.length }));

      if (!res || res.status === false) throw new Error(res?.message || "Failed to load");

      setProjectDropdown(res.project_dropdown || []);

      // ── KPI cards — directly from API summary ──────────────
      const s = res.summary || {};
      setSummary({
        total_tickets: s.total_tickets || 0,
        active_tickets: s.active_tickets || 0,
        resolved_tickets: s.resolved_tickets || 0,
        closed_tickets: s.closed_tickets || 0,
        total_projects: s.total_projects || 0,
      });

      // ── Build lookup: lookup[month][project_name] ──────────
      const newLookup = {};
      let max = 1;

      (res.months || []).forEach((monthObj) => {
        newLookup[monthObj.month] = {};
        (monthObj.projects || []).forEach((p) => {
          const count = p.overall?.total ?? 0;
          const bd = {
            open: p.overall?.open || 0,
            pending: p.overall?.pending || 0,
            answered: p.overall?.answered || 0,
            resolved: p.overall?.resolved || 0,
            closed: p.overall?.closed || 0,
          };
          // Key by project_name (string) — same as what backend sends
          newLookup[monthObj.month][p.project_name] = {
            total: count,
            breakdown: bd,
            tickets_by_status: p.tickets_by_status || {},
          };
      ...
      // ── Build projects list from dropdown ──────────────────
      const allProjects = (res.project_dropdown || []).map((p) => ({
        id: p.project_id,
        name: p.project_name,
      }));

      // Sort: projects with any tickets first (by grand total desc)
      allProjects.sort((a, b) => {
        const ta = MONTHS.reduce((s, m) => s + (newLookup[m]?.[a.name]?.total || 0), 0);
        const tb = MONTHS.reduce((s, m) => s + (newLookup[m]?.[b.name]?.total || 0), 0);
        return tb - ta;
      });
  ...
  const handleCellClick = (proj, month) => {
    const cell = lookup[month]?.[proj.name];
    ...
    setModalData({
      ...
      tickets_by_status: cell.tickets_by_status,
    });
    setModalOpen(true);
  };

  // ── Status row click → populate drawer from heatmap data ──
  // No extra API call — data already exists in lookup
  const handleStatusClick = (statusKey) => {
    if (!modalData) return;
    const tickets = modalData.tickets_by_status?.[statusKey] || [];
    setDrawerTitle("Showing Tickets For Status: ");
  ...
  // ── KPI values direct from API summary ────────────────────
  const totalByMonth = MONTHS.map((m) =>
    projects.reduce((s, p) => s + (lookup[m]?.[p.name]?.total || 0), 0)
  );
  ...
  const kpis = [
    { title: "Total Tickets", value: summary.total_tickets, color: "#4F46E5", icon: <TicketIcon /> },
    { title: "Active Tickets", value: summary.active_tickets, color: "#F59E0B", icon: <FolderIcon /> },
    { title: "Resolved Tickets", value: summary.resolved_tickets, color: "#8B5CF6", icon: <ResolvedIcon /> },
    { title: "Closed Tickets", value: summary.closed_tickets, color: "#EF4444", icon: <TicketIcon /> },
    { title: "Projects", value: summary.total_projects, color: "#10B981", icon: <PeakIcon /> },
  ];
      ...
      <MotionBox initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}>
        ...
        {loading ? (
          ...
          <Paper elevation={0} sx={{ borderRadius: "16px", overflow: "hidden", border: "1px solid", borderColor: border, boxShadow: isDark ? "0 4px 20px rgba(0,0,0,0.3)" : "0 15px 35px -10px rgba(148,163,184,0.12)" }}>
            <Box sx={{
              overflowX: "auto", maxHeight: "calc(100vh - 440px)", overflowY: "auto",
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                ...
                <tbody>
                  {projects.map((proj, idx) => {
                    const rowTotal = MONTHS.reduce((s, m) => s + (lookup[m]?.[proj.name]?.total || 0), 0);
                        ...
                        {MONTHS.map((m) => {
                          ...
                          return (
                            <td key={m} style={{ padding: "5px 3px", textAlign: "center", borderBottom: `1px solid ${heatBorder}`, borderLeft: m === currentMonth ? "2px solid #6366F133" : "none" }}>
                              ...
                              >
                                <Box
                                  onClick={() => handleCellClick(proj, m)}
